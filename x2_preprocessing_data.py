...
class PreprocessingData:
    ...
    def cleaning_data(self):
        self.data = self.data.loc[
            self.data["Point of Contact"] != "Contact Builder"
        ].copy()
    ...
    def transform_data(self):
        self.data[["floor_placed", "floor_number"]] = self.data["Floor"].str.split(
            " out of ", expand=True
        )
        ...
        self.data.loc[
            self.data["floor_placed"] == "Upper Basement", "floor_placed"
        ] = self.data.loc[self.data["floor_placed"] == "Upper Basement", "floor_number"]
    ...
    def create_dummies(self):
        for i in self.cat_dummies:
            self.data = pd.concat(
                [self.data, pd.get_dummies(self.data[i], prefix=f"{i}")], axis=1
            )
    ...
    def export_data_cleaned(self):
        self.data.to_csv("data_clean.csv", index=False)
...
if __name__ == "__main__":
    ...
    preprocess.export_data_cleaned()
